export type Language = 'en' | 'tr';

export const translations = {
/* --------------------
   HERO
-------------------- */

homeHeroKicker: {
  en: 'INERTIAL INTELLIGENCE',
  tr: 'ATALETSEL ZEKA',
},

homeHeroTitle: {
  en: 'Inertial Intelligence for Real-World Systems',
  tr: 'Fiziksel Sistemler İçin Ataletsel Zeka',
},

homeHeroLead: {
  en: 'Ayrota develops inertial navigation, motion estimation and operational intelligence technologies for autonomous systems, vehicles and indoor environments.',
  tr: 'Ayrota; otonom sistemler, araçlar ve kapalı alan uygulamaları için ataletsel navigasyon, hareket kestirimi ve operasyonel analiz teknolojileri geliştirir.',
},

homeHeroPrimaryCta: {
  en: 'APPLICATIONS',
  tr: 'UYGULAMA ALANLARI',
},

homeHeroVisualStatus: {
  en: 'AYROTA',
  tr: 'AYROTA',
},

homeHeroVisualKicker: {
  en: 'ONE TECHNOLOGY CORE',
  tr: 'ORTAK TEKNOLOJİ ÇEKİRDEĞİ',
},

homeHeroVisualText: {
  en: 'Navigation, movement estimation and analytics from a shared inertial technology foundation.',
  tr: 'Navigasyon, hareket kestirimi ve analitik için ortak ataletsel teknoloji altyapısı.',
},
 /* --------------------
   PRODUCTS
-------------------- */

productsKicker: {
  en: 'PRODUCTS',
  tr: 'ÜRÜNLER',
},

productsTitle: {
  en: 'Inertial Intelligence, Applied.',
  tr: 'Ataletsel Zekanın Ürünleşmiş Hali.',
},

productsLead: {
  en: 'Ayrota turns its inertial navigation, movement estimation and motion-analysis capabilities into products for autonomous systems, indoor operations and road intelligence.',
  tr: 'Ayrota; ataletsel navigasyon, hareket kestirimi ve hareket analizi kabiliyetlerini otonom sistemler, kapalı alan operasyonları ve yol analitiği için ürünlere dönüştürür.',
},

productsDetail: {
  en: 'EXPLORE PRODUCT',
  tr: 'ÜRÜNÜ İNCELE',
},

/* --------------------
   ALKON
-------------------- */

productsAlkonEyebrow: {
  en: 'GNSS-DENIED NAVIGATION',
  tr: 'GNSS KISITLI NAVİGASYON',
},

productsAlkonDescription: {
  en: 'IMU-based navigation capability for autonomous platforms operating where GNSS cannot be fully relied upon.',
  tr: 'GNSS’in güvenilir olmadığı koşullarda çalışan otonom platformlar için IMU tabanlı navigasyon kabiliyeti.',
},

productsAlkonApplicationOne: {
  en: 'UAV & UGV',
  tr: 'İHA & İKA',
},

productsAlkonApplicationTwo: {
  en: 'AUTONOMOUS SYSTEMS',
  tr: 'OTONOM SİSTEMLER',
},

/* --------------------
   YAYA
-------------------- */

productsYayaEyebrow: {
  en: 'INDOOR MOVEMENT',
  tr: 'KAPALI ALAN HAREKETİ',
},

productsYayaDescription: {
  en: 'Estimates relative pedestrian movement from mobile inertial sensors and turns indoor motion into route, coverage and operational insights.',
  tr: 'Mobil ataletsel sensörlerden göreli yaya hareketini kestirir; kapalı alan hareketini rota, kapsama ve operasyonel içgörülere dönüştürür.',
},

productsYayaApplicationOne: {
  en: 'HOSPITALS',
  tr: 'HASTANELER',
},

productsYayaApplicationTwo: {
  en: 'DEPOTS & FACILITIES',
  tr: 'DEPOLAR & TESİSLER',
},

/* --------------------
   YÖRÜK
-------------------- */

productsYorukEyebrow: {
  en: 'ROAD & DRIVING ANALYTICS',
  tr: 'YOL & SÜRÜŞ ANALİTİĞİ',
},

productsYorukDescription: {
  en: 'Transforms vehicle movement data into road-condition, maintenance and driving-safety insights for operational decision making.',
  tr: 'Araç hareket verisini yol durumu, bakım ve sürüş güvenliğine ilişkin operasyonel içgörülere dönüştürür.',
},

productsYorukApplicationOne: {
  en: 'ROAD MAINTENANCE',
  tr: 'YOL BAKIMI',
},

productsYorukApplicationTwo: {
  en: 'DRIVING SAFETY',
  tr: 'SÜRÜŞ GÜVENLİĞİ',
},
  /* --------------------
   APPLICATIONS
-------------------- */

applicationsKicker: {
  en: 'APPLICATION AREAS',
  tr: 'UYGULAMA ALANLARI',
},

applicationsTitle: {
  en: 'From a Single Technology Core to Diverse Application Areas.',
  tr: 'Tek Teknoloji Çekirdeğinden Farklı Uygulama Alanlarına.',
},

applicationsLead: {
  en: 'Ayrota adapts its inertial technology across defence, dual-use and civil applications.',
  tr: 'Ayrota, ataletsel teknoloji altyapısını savunma, çift kullanım ve sivil uygulamalara uyarlar.',
},

applicationDefenseTitle: {
  en: 'Defence',
  tr: 'Savunma',
},

applicationDefenseText: {
  en: 'Navigation and motion capabilities for autonomous platforms operating in GNSS-constrained and mission-critical environments.',
  tr: 'GNSS erişiminin kısıtlandığı ve görev sürekliliğinin kritik olduğu ortamlarda çalışan otonom platformlar için navigasyon ve hareket kabiliyetleri.',
},

applicationDefenseDetail: {
  en: 'UAV · Autonomous Systems',
  tr: 'İHA · Otonom Sistemler',
},

applicationDualUseTitle: {
  en: 'Dual-Use',
  tr: 'Çift Kullanım',
},

applicationDualUseText: {
  en: 'Inertial navigation and movement intelligence for robotics, critical infrastructure and industrial systems.',
  tr: 'Robotik sistemler, kritik altyapılar ve endüstriyel uygulamalar için ataletsel navigasyon ve hareket analizi.',
},

applicationDualUseDetail: {
  en: 'Robotics · Critical Infrastructure · Industry',
  tr: 'Robotik · Kritik Altyapı · Endüstri',
},

applicationCivilTitle: {
  en: 'Civil & Commercial',
  tr: 'Sivil ve Ticari',
},

applicationCivilText: {
  en: 'Motion intelligence for road infrastructure, fleets, facilities and urban operations.',
  tr: 'Yol altyapısı, filolar, tesisler ve şehir operasyonları için hareket verisine dayalı analiz ve karar desteği.',
},

applicationCivilDetail: {
  en: 'Roads · Fleets · Facilities · Smart Cities',
  tr: 'Yol · Filo · Tesis · Akıllı Şehir',
},

applicationsBottomText: {
  en: 'The same inertial engineering foundation is adapted according to platform, sensor and operational requirements.',
  tr: 'Aynı ataletsel mühendislik altyapısı; platforma, sensöre ve operasyonel ihtiyaca göre farklı çözümlere uyarlanır.',
},

applicationsTechnologyCta: {
  en: 'EXPLORE THE TECHNOLOGY',
  tr: 'TEKNOLOJİYİ İNCELE',
},
  /* --------------------
   CORE TECHNOLOGY
-------------------- */

technologyKicker: {
  en: 'CORE TECHNOLOGY',
  tr: 'TEMEL TEKNOLOJİ',
},

technologyTitle: {
  en: 'One Inertial Technology Core',
  tr: 'Tek Bir Ataletsel Teknoloji Çekirdeği',
},

technologyLead: {
  en: 'Ayrota combines inertial sensing, motion estimation, navigation, movement analytics and edge integration within a shared technology foundation.',
  tr: 'Ayrota; ataletsel algılama, hareket kestirimi, navigasyon, hareket analitiği ve edge entegrasyonunu ortak bir teknoloji altyapısında birleştirir.',
},

technologyCoreTitle: {
  en: 'Shared Inertial Technology Foundation',
  tr: 'Ortak Ataletsel Teknoloji Altyapısı',
},

technologyCoreMetricSensing: {
  en: 'SENSING LAYER',
  tr: 'ALGILAMA KATMANI',
},

technologyCoreMetricSensingValue: {
  en: 'INERTIAL DATA',
  tr: 'ATALETSEL VERİ',
},

technologyCoreMetricEstimation: {
  en: 'ESTIMATION LAYER',
  tr: 'KESTİRİM KATMANI',
},

technologyCoreMetricEstimationValue: {
  en: 'NAVIGATION & MOTION',
  tr: 'NAVİGASYON & HAREKET',
},

technologyCoreMetricIntegration: {
  en: 'SYSTEM LAYER',
  tr: 'SİSTEM KATMANI',
},

technologyCoreMetricIntegrationValue: {
  en: 'EDGE & INTEGRATION',
  tr: 'EDGE & ENTEGRASYON',
},

technologyNavigationTitle: {
  en: 'Navigation',
  tr: 'Navigasyon',
},

technologyNavigationText: {
  en: 'IMU-based navigation and navigation resilience for autonomous platforms operating under GNSS-constrained conditions.',
  tr: 'GNSS kısıtlı koşullarda çalışan otonom platformlar için IMU tabanlı navigasyon ve navigasyon sürekliliği.',
},

technologySensorTitle: {
  en: 'Inertial Sensing',
  tr: 'Ataletsel Algılama',
},

technologySensorText: {
  en: 'Processing inertial measurements into reliable inputs for navigation, movement estimation and motion analytics.',
  tr: 'Ataletsel ölçümlerin navigasyon, hareket kestirimi ve hareket analitiği için kullanılabilir verilere dönüştürülmesi.',
},

technologyIndoorTitle: {
  en: 'Movement Estimation',
  tr: 'Hareket Kestirimi',
},

technologyIndoorText: {
  en: 'Relative movement estimation and motion analysis from mobile inertial sensors in GNSS-constrained environments.',
  tr: 'GNSS’in kısıtlı olduğu ortamlarda mobil ataletsel sensörlerden göreli hareket kestirimi ve hareket analizi.',
},

technologyAnalyticsTitle: {
  en: 'Motion Analytics',
  tr: 'Hareket Analitiği',
},

technologyAnalyticsText: {
  en: 'Transforming vehicle and pedestrian motion data into operational insights for roads, facilities and mobility applications.',
  tr: 'Araç ve yaya hareket verisinin yol, tesis ve mobilite uygulamaları için operasyonel içgörülere dönüştürülmesi.',
},

technologyEmbeddedTitle: {
  en: 'System Integration',
  tr: 'Sistem Entegrasyonu',
},

technologyEmbeddedText: {
  en: 'Deploying inertial processing and estimation capabilities across embedded, mobile and autonomous-system environments.',
  tr: 'Ataletsel veri işleme ve kestirim kabiliyetlerinin gömülü, mobil ve otonom sistem ortamlarına taşınması.',
},

technologyBottomKicker: {
  en: 'SHARED FOUNDATION',
  tr: 'ORTAK ALTYAPI',
},

technologyBottomText: {
  en: 'The same inertial technology core supports Alkon, Yaya and Yörük across navigation, movement and analytics.',
  tr: 'Aynı ataletsel teknoloji çekirdeği; Alkon, Yaya ve Yörük ürünlerini navigasyon, hareket ve analitik eksenlerinde destekler.',
},
/* --------------------
   ENGAGEMENT
-------------------- */

engagementKicker: {
  en: 'WORKING WITH AYROTA',
  tr: 'AYROTA İLE ÇALIŞMA',
},

engagementTitle: {
  en: 'From Validation to Deployment.',
  tr: 'Doğrulamadan Uygulamaya.',
},

engagementLead: {
  en: 'Ayrota products are introduced through controlled evaluation, pilot and integration stages according to the product and operating environment.',
  tr: 'Ayrota ürünleri; ürüne ve çalışma ortamına göre kontrollü değerlendirme, pilot ve entegrasyon aşamalarıyla devreye alınır.',
},

engagementAssessmentTitle: {
  en: 'Define',
  tr: 'Tanımla',
},

engagementAssessmentText: {
  en: 'The platform, operating environment, available data and expected outcome are defined.',
  tr: 'Platform, çalışma ortamı, mevcut veri ve beklenen çıktı netleştirilir.',
},

engagementPocTitle: {
  en: 'Validate',
  tr: 'Doğrula',
},

engagementPocText: {
  en: 'The product capability is evaluated under a controlled and measurable scenario.',
  tr: 'Ürün kabiliyeti kontrollü ve ölçülebilir bir senaryo altında değerlendirilir.',
},

engagementPilotTitle: {
  en: 'Pilot',
  tr: 'Pilot',
},

engagementPilotText: {
  en: 'The validated capability is tested in a representative operational environment and workflow.',
  tr: 'Doğrulanan kabiliyet gerçek kullanımı temsil eden bir operasyon ortamı ve iş akışı içinde test edilir.',
},

engagementIntegrationTitle: {
  en: 'Integrate',
  tr: 'Entegre Et',
},

engagementIntegrationText: {
  en: 'The product is integrated with the target platform, application or operational software stack.',
  tr: 'Ürün, hedef platforma, uygulamaya veya operasyonel yazılım altyapısına entegre edilir.',
},

engagementScaleTitle: {
  en: 'Operate',
  tr: 'Kullan',
},

engagementScaleText: {
  en: 'Validated deployments can continue through software delivery, recurring analytics, support or licensing models depending on the product.',
  tr: 'Doğrulanmış uygulamalar, ürüne göre yazılım teslimi, tekrarlayan analitik, destek veya lisanslama modelleriyle sürdürülebilir.',
},

engagementBottomText: {
  en: 'The exact path depends on the product and operating scenario; not every engagement needs to pass through every stage.',
  tr: 'İzlenecek yol ürüne ve operasyon senaryosuna göre değişir; her çalışma bütün aşamalardan geçmek zorunda değildir.',
},

  /* --------------------
   CONTACT
-------------------- */

contactKicker: {
  en: 'CONTACT',
  tr: 'İLETİŞİM',
},

contactTitle: {
  en: 'Let’s Discuss Your Application.',
  tr: 'Uygulamanızı Birlikte Değerlendirelim.',
},

contactLead: {
  en: 'Tell us about your platform, operating environment or use case. We can evaluate how Alkon, Yaya or Yörük fits your application.',
  tr: 'Platformunuzu, çalışma ortamınızı veya kullanım senaryonuzu paylaşın. Ürünlerimizin uygulamanıza nasıl uyabileceğini birlikte değerlendirelim.',
},

contactProjectKicker: {
  en: 'WHAT TO SHARE',
  tr: 'NE PAYLAŞABİLİRSİNİZ',
},

contactProjectText: {
  en: 'A short description of your operating environment, platform, available data and expected outcome is enough for an initial discussion.',
  tr: 'İlk görüşme için çalışma ortamınızı, platformunuzu, mevcut veriyi ve beklediğiniz çıktıyı kısaca anlatmanız yeterlidir.',
},

contactLabelOrg: {
  en: 'PERSON / ORGANISATION',
  tr: 'AD / KURUM',
},

contactLabelEmail: {
  en: 'EMAIL',
  tr: 'E-POSTA',
},

contactLabelProduct: {
  en: 'PRODUCT',
  tr: 'ÜRÜN',
},

contactLabelMessage: {
  en: 'APPLICATION / MESSAGE',
  tr: 'UYGULAMA / MESAJ',
},

contactPlaceholderOrg: {
  en: 'Enter your name or organisation',
  tr: 'Adınızı veya kurumunuzu yazın',
},

contactPlaceholderEmail: {
  en: 'Enter your email address',
  tr: 'E-posta adresinizi yazın',
},

contactPlaceholderProduct: {
  en: 'Select a product',
  tr: 'Ürün seçin',
},

contactProductUnsure: {
  en: 'OTHER',
  tr: 'DİĞER',
},

contactPlaceholderMessage: {
  en: 'Tell us briefly about your platform, environment or use case...',
  tr: 'Platformunuzu, çalışma ortamınızı veya kullanım senaryonuzu kısaca anlatın...',
},

contactSubmit: {
  en: 'SEND MESSAGE',
  tr: 'MESAJI GÖNDER',
},

contactPrivacyNote: {
  en: 'Your information is used only to evaluate and respond to your inquiry.',
  tr: 'Paylaştığınız bilgiler yalnızca talebinizi değerlendirmek ve size dönüş yapmak amacıyla kullanılır.',
},

contactLocationKicker: {
  en: 'LOCATION',
  tr: 'KONUM',
},

contactLocationText: {
  en: 'Ankara, Türkiye',
  tr: 'Ankara, Türkiye',
},

contactMapTitle: {
  en: 'Ayrota office location',
  tr: 'Ayrota ofis konumu',
},
  /* --------------------
     FOOTER
  -------------------- */

  footerCopyright: {
    en: '© {{year}} Ayrota',
    tr: '© {{year}} Ayrota',
  },

 footerDescription: {
  en: 'Inertial navigation, movement estimation and operational intelligence for autonomous systems, vehicles and indoor environments.',
  tr: 'Otonom sistemler, araçlar ve kapalı alan uygulamaları için ataletsel navigasyon, hareket kestirimi ve operasyonel analiz teknolojileri.',
},

  footerPrivacy: {
    en: 'Privacy & Data Notice',
    tr: 'Gizlilik ve Veri Bildirimi',
  },

 /* --------------------
   ALKON PAGE
-------------------- */

alkonProductCategory: {
  en: 'GNSS-DENIED NAVIGATION',
  tr: 'GNSS KISITLI NAVİGASYON',
},

alkonHeroStatement: {
  en: 'Navigation Capability When GNSS Cannot Be Relied Upon',
  tr: 'GNSS Güvenilir Olmadığında Navigasyon Kabiliyeti',
},

alkonHeroText: {
  en: 'Alkon is Ayrota’s GNSS-denied navigation capability for autonomous platforms, combining inertial navigation, controlled validation and platform integration.',
  tr: 'Alkon, otonom platformlar için geliştirilen GNSS kısıtlı navigasyon kabiliyetidir; ataletsel navigasyon, kontrollü doğrulama ve platform entegrasyonunu bir araya getirir.',
},

alkonMiniNavigation: {
  en: 'IMU-BASED NAVIGATION',
  tr: 'IMU TABANLI NAVİGASYON',
},

alkonMiniAutonomy: {
  en: 'AUTONOMOUS PLATFORMS',
  tr: 'OTONOM PLATFORMLAR',
},

alkonMiniIntegration: {
  en: 'SYSTEM INTEGRATION',
  tr: 'SİSTEM ENTEGRASYONU',
},

alkonVisualKicker: {
  en: 'AUTONOMOUS SYSTEMS',
  tr: 'OTONOM SİSTEMLER',
},

alkonVisualText: {
  en: 'Navigation resilience under GNSS-denied conditions.',
  tr: 'GNSS kısıtlı koşullarda navigasyon sürekliliği.',
},

alkonProblemKicker: {
  en: 'THE PROBLEM',
  tr: 'PROBLEM',
},

alkonProblemTitle: {
  en: 'Autonomy Becomes Fragile When GNSS Becomes Unreliable.',
  tr: 'GNSS Güvenilirliğini Kaybettiğinde Otonomi Kırılganlaşır.',
},

alkonProblemText: {
  en: 'Autonomous platforms can lose navigation capability when GNSS is degraded, jammed, spoofed or unavailable. Inertial navigation provides continuity, but its error behaviour must be understood and managed over mission-relevant periods.',
  tr: 'Otonom platformlar GNSS zayıfladığında, karıştırıldığında, yanıltıldığında veya tamamen kullanılamadığında navigasyon kabiliyetini kaybedebilir. Ataletsel navigasyon süreklilik sağlar; ancak hata davranışının görev açısından anlamlı süreler boyunca anlaşılması ve yönetilmesi gerekir.',
},

alkonProblemNote: {
  en: 'Alkon focuses on measurable navigation behaviour and integration risk rather than absolute or zero-drift claims.',
  tr: 'Alkon, mutlak doğruluk veya sıfır drift iddiası yerine ölçülebilir navigasyon davranışı ve entegrasyon riskine odaklanır.',
},

alkonScopeTitle: {
  en: 'CAPABILITY',
  tr: 'KABİLİYET',
},

alkonScopeMainTitle: {
  en: 'A GNSS-Denied Navigation Layer for Autonomous Platforms',
  tr: 'Otonom Platformlar İçin GNSS Kısıtlı Navigasyon Katmanı',
},

alkonScopeText: {
  en: 'Alkon combines IMU-based navigation with autonomous-platform integration to evaluate navigation continuity when external positioning cannot be relied upon.',
  tr: 'Alkon, harici konumlama kaynaklarına güvenilemediği koşullarda navigasyon sürekliliğini değerlendirmek için IMU tabanlı navigasyonu otonom platform entegrasyonuyla birleştirir.',
},

alkonCapabilityNavigationTitle: {
  en: 'Inertial Navigation',
  tr: 'Ataletsel Navigasyon',
},

alkonCapabilityNavigationText: {
  en: 'IMU-based navigation state estimation during periods where GNSS measurements are degraded or unavailable.',
  tr: 'GNSS ölçümlerinin zayıfladığı veya kullanılamadığı dönemlerde IMU verisinden navigasyon durumlarının kestirilmesi.',
},

alkonCapabilityResilienceTitle: {
  en: 'Navigation Resilience',
  tr: 'Navigasyon Sürekliliği',
},

alkonCapabilityResilienceText: {
  en: 'Controlled evaluation of navigation behaviour, drift and mission-relevant performance under GNSS-denied conditions.',
  tr: 'GNSS kısıtlı koşullarda navigasyon davranışının, drift karakterinin ve görev açısından anlamlı performansın kontrollü olarak değerlendirilmesi.',
},

alkonCapabilityIntegrationTitle: {
  en: 'Platform Integration',
  tr: 'Platform Entegrasyonu',
},

alkonCapabilityIntegrationText: {
  en: 'Integration with autonomous-system software and robotics stacks according to the target platform.',
  tr: 'Hedef platforma göre otonom sistem yazılımı ve robotik altyapılarla entegrasyon.',
},

alkonPocKicker: {
  en: 'TECHNICAL POC',
  tr: 'TEKNİK POC',
},

alkonPocTitle: {
  en: 'Validate Before Deployment.',
  tr: 'Entegrasyondan Önce Doğrula.',
},

alkonPocLead: {
  en: 'Alkon is currently available through controlled technical PoC and integration-assessment studies.',
  tr: 'Alkon bugün kontrollü teknik PoC ve entegrasyon değerlendirme çalışmaları kapsamında sunulur.',
},

alkonPocScenarioTitle: {
  en: 'Scenario Definition',
  tr: 'Senaryo Tanımı',
},

alkonPocScenarioText: {
  en: 'Platform, operating conditions, available sensors and success criteria are defined together.',
  tr: 'Platform, çalışma koşulları, mevcut sensörler ve başarı ölçütleri birlikte netleştirilir.',
},

alkonPocExperimentTitle: {
  en: 'Navigation Experiment',
  tr: 'Navigasyon Deneyi',
},

alkonPocExperimentText: {
  en: 'IMU-based navigation behaviour is evaluated under a constrained and measurable technical scenario.',
  tr: 'IMU tabanlı navigasyon davranışı sınırlı ve ölçülebilir bir teknik senaryo altında değerlendirilir.',
},

alkonPocReportTitle: {
  en: 'Engineering Report',
  tr: 'Mühendislik Raporu',
},

alkonPocReportText: {
  en: 'Drift behaviour, integration risks and recommended next steps are documented for technical decision making.',
  tr: 'Drift davranışı, entegrasyon riskleri ve önerilen sonraki adımlar teknik karar sürecini destekleyecek şekilde raporlanır.',
},

alkonValidationKicker: {
  en: 'CURRENT VALIDATION',
  tr: 'MEVCUT DOĞRULAMA',
},

alkonValidationTitle: {
  en: 'Validation Progress',
  tr: 'Doğrulama Durumu',
},

alkonValidationSimulation: {
  en: 'SIMULATION',
  tr: 'SİMÜLASYON',
},

alkonValidationRobustness: {
  en: 'ROBUSTNESS',
  tr: 'DAYANIKLILIK',
},

alkonValidationHitl: {
  en: 'HITL / FIELD',
  tr: 'HITL / SAHA',
},

alkonValidationComplete: {
  en: 'COMPLETE',
  tr: 'TAMAMLANDI',
},

alkonValidationOngoing: {
  en: 'IN PROGRESS',
  tr: 'DEVAM EDİYOR',
},

alkonValidationNext: {
  en: 'NEXT VALIDATION STEP',
  tr: 'SONRAKİ DOĞRULAMA ADIMI',
},

alkonFinalCtaTitle: {
  en: 'Evaluate GNSS-Denied Navigation for Your Platform.',
  tr: 'Platformunuz İçin GNSS Kısıtlı Navigasyonu Değerlendirelim.',
},

alkonFinalCtaText: {
  en: 'Share your platform, navigation stack, sensors and operating scenario. We can define an appropriate technical evaluation or PoC scope.',
  tr: 'Platformunuzu, navigasyon altyapınızı, sensörlerinizi ve çalışma senaryonuzu paylaşın. Uygun teknik değerlendirme veya PoC kapsamını birlikte belirleyelim.',
},

alkonCtaButton: {
  en: 'DISCUSS ALKON',
  tr: 'ALKON’U GÖRÜŞELİM',
},

/* --------------------
   YORUK PAGE
-------------------- */

yorukProductCategory: {
  en: 'ROAD & DRIVING ANALYTICS',
  tr: 'YOL VE SÜRÜŞ ANALİTİĞİ',
},

yorukHeroStatement: {
  en: 'Turn Vehicle Motion Into Road and Driving Intelligence',
  tr: 'Araç Hareketini Yol ve Sürüş Bilgisine Dönüştürün',
},

yorukHeroText: {
  en: 'Yörük uses vehicle motion data to produce road-condition, maintenance and driving-safety insights for operational decision making.',
  tr: 'Yörük, araç hareket verisini kullanarak yol durumu, bakım ve sürüş güvenliği hakkında operasyonel karar süreçlerini destekleyen bilgiler üretir.',
},

yorukMiniRoad: {
  en: 'ROAD CONDITION',
  tr: 'YOL DURUMU',
},

yorukMiniDriving: {
  en: 'DRIVING SAFETY',
  tr: 'SÜRÜŞ GÜVENLİĞİ',
},

yorukMiniSensors: {
  en: 'GPS + IMU',
  tr: 'GPS + IMU',
},

yorukVisualKicker: {
  en: 'VEHICLE MOTION INTELLIGENCE',
  tr: 'ARAÇ HAREKET ZEKASI',
},

yorukVisualText: {
  en: 'Operational insight from vehicle GPS and inertial data.',
  tr: 'Araç GPS ve ataletsel verisinden operasyonel bilgi.',
},

yorukProblemKicker: {
  en: 'THE PROBLEM',
  tr: 'PROBLEM',
},

yorukProblemTitle: {
  en: 'Road and Driving Conditions Are Difficult to Observe Continuously.',
  tr: 'Yol ve Sürüş Koşullarını Sürekli Gözlemlemek Zordur.',
},

yorukProblemText: {
  en: 'Road networks and vehicle operations generate continuous physical movement, but conventional inspection and reporting processes provide only limited snapshots. Vehicle sensor data can turn normal operations into a continuous source of road and driving information.',
  tr: 'Yol ağları ve araç operasyonları sürekli fiziksel hareket üretir; ancak klasik denetim ve raporlama yöntemleri çoğu zaman yalnızca sınırlı anlık görüntüler sağlar. Araç sensör verisi, günlük operasyonları sürekli bir yol ve sürüş bilgisi kaynağına dönüştürebilir.',
},

yorukScopeTitle: {
  en: 'CAPABILITY',
  tr: 'KABİLİYET',
},

yorukScopeMainTitle: {
  en: 'Road and Driving Analytics From Vehicle Motion',
  tr: 'Araç Hareketinden Yol ve Sürüş Analitiği',
},

yorukScopeText: {
  en: 'Yörük processes vehicle GPS and IMU data to identify road characteristics, driving events and operational patterns across repeated trips.',
  tr: 'Yörük, tekrar eden araç hareketlerinden gelen GPS ve IMU verilerini işleyerek yol özelliklerini, sürüş olaylarını ve operasyonel örüntüleri değerlendirir.',
},

yorukCapabilityRoadTitle: {
  en: 'Road Condition',
  tr: 'Yol Durumu',
},

yorukCapabilityRoadText: {
  en: 'Vehicle motion can be analysed to identify road-condition differences and support maintenance prioritisation.',
  tr: 'Araç hareketi analiz edilerek yol koşullarındaki farklılıklar belirlenebilir ve bakım önceliklendirmesi desteklenebilir.',
},

yorukCapabilityDrivingTitle: {
  en: 'Driving Behaviour',
  tr: 'Sürüş Davranışı',
},

yorukCapabilityDrivingText: {
  en: 'Motion signals can be transformed into driving-risk events and behaviour patterns for safety analysis.',
  tr: 'Hareket sinyalleri sürüş güvenliği analizi için risk olaylarına ve davranış örüntülerine dönüştürülebilir.',
},

yorukCapabilityOperationalTitle: {
  en: 'Operational Analytics',
  tr: 'Operasyonel Analitik',
},

yorukCapabilityOperationalText: {
  en: 'Repeated trips provide route- and fleet-level data that can support comparison, reporting and operational decisions.',
  tr: 'Tekrar eden seferler; rota ve filo seviyesinde karşılaştırma, raporlama ve operasyonel karar desteği için veri sağlar.',
},

yorukApplicationsKicker: {
  en: 'APPLICATION SURFACES',
  tr: 'KULLANIM ALANLARI',
},

yorukApplicationsTitle: {
  en: 'Two Operational Uses of the Same Vehicle Data.',
  tr: 'Aynı Araç Verisinin İki Operasyonel Kullanımı.',
},

yorukApplicationsLead: {
  en: 'The same vehicle motion data can support infrastructure maintenance and driving-safety workflows.',
  tr: 'Aynı araç hareket verisi hem altyapı bakım süreçlerini hem de sürüş güvenliği çalışmalarını destekleyebilir.',
},

yorukRoadMaintenanceTitle: {
  en: 'Road Maintenance',
  tr: 'Yol Bakım',
},

yorukRoadMaintenanceText: {
  en: 'Existing fleet vehicles can act as a moving road-observation network. Repeated measurements support road-condition monitoring, maintenance prioritisation and periodic reporting.',
  tr: 'Mevcut filo araçları hareket eden bir yol gözlem ağı gibi kullanılabilir. Tekrar eden ölçümler yol durumunun izlenmesini, bakım önceliklendirmesini ve periyodik raporlamayı destekler.',
},

yorukSafetyTitle: {
  en: 'Driving Safety',
  tr: 'Sürüş Güvenliği',
},

yorukSafetyText: {
  en: 'Vehicle IMU data can be analysed at driver and trip level to identify risk events and recurring driving-behaviour patterns.',
  tr: 'Araç IMU verisi sürücü ve sefer seviyesinde analiz edilerek riskli sürüş olayları ve tekrar eden sürüş davranışı örüntüleri belirlenebilir.',
},

yorukFlowKicker: {
  en: 'DATA FLOW',
  tr: 'VERİ AKIŞI',
},

yorukFlowTitle: {
  en: 'From Everyday Vehicle Motion to Operational Output.',
  tr: 'Günlük Araç Hareketinden Operasyonel Çıktıya.',
},

yorukFlowCollectTitle: {
  en: 'Collect',
  tr: 'Topla',
},

yorukFlowCollectText: {
  en: 'GPS and IMU data is collected during normal vehicle operation.',
  tr: 'GPS ve IMU verisi aracın normal operasyonu sırasında toplanır.',
},

yorukFlowAnalyseTitle: {
  en: 'Analyse',
  tr: 'Analiz Et',
},

yorukFlowAnalyseText: {
  en: 'Motion signals are processed to identify road and driving characteristics.',
  tr: 'Hareket sinyalleri işlenerek yol ve sürüş özellikleri değerlendirilir.',
},

yorukFlowOutputTitle: {
  en: 'Report',
  tr: 'Raporla',
},

yorukFlowOutputText: {
  en: 'Findings are converted into maps, comparisons and operational reports according to the use case.',
  tr: 'Bulgular kullanım senaryosuna göre harita, karşılaştırma ve operasyonel raporlara dönüştürülür.',
},

yorukFinalCtaTitle: {
  en: 'Evaluate Yörük for Your Vehicle Operations.',
  tr: 'Yörük’ü Araç Operasyonlarınız İçin Değerlendirelim.',
},

yorukFinalCtaText: {
  en: 'Tell us about your vehicles, routes and the road-maintenance or driving-safety problem you want to evaluate.',
  tr: 'Araçlarınızı, rotalarınızı ve değerlendirmek istediğiniz yol bakım veya sürüş güvenliği problemini paylaşın. Uygun kullanım kapsamını birlikte belirleyelim.',
},

yorukCtaButton: {
  en: 'DISCUSS YÖRÜK',
  tr: 'YÖRÜK’Ü GÖRÜŞELİM',
},
/* --------------------
   YAYA PAGE
-------------------- */

yayaProductCategory: {
  en: 'INDOOR MOVEMENT INTELLIGENCE',
  tr: 'KAPALI ALAN HAREKET ANALİTİĞİ',
},

yayaHeroStatement: {
  en: 'Understand Indoor Movement From Mobile Sensors',
  tr: 'Mobil Sensörlerden Kapalı Alan Hareketini Anlayın',
},

yayaHeroText: {
  en: 'Yaya uses mobile inertial sensors to estimate relative pedestrian movement and turn indoor motion into route, coverage and operational insights.',
  tr: 'Yaya, mobil ataletsel sensörlerle göreli yaya hareketini kestirir ve kapalı alan hareketini rota, kapsama ve operasyonel içgörülere dönüştürür.',
},

yayaMiniMovement: {
  en: 'MOVEMENT ESTIMATION',
  tr: 'HAREKET KESTİRİMİ',
},

yayaMiniRoute: {
  en: 'ROUTE & COVERAGE',
  tr: 'ROTA & KAPSAMA',
},

yayaMiniMobile: {
  en: 'MOBILE IMU',
  tr: 'MOBİL IMU',
},

yayaVisualKicker: {
  en: 'INDOOR MOVEMENT INTELLIGENCE',
  tr: 'KAPALI ALAN HAREKET ANALİTİĞİ',
},

yayaVisualText: {
  en: 'Relative movement and route intelligence from mobile inertial sensing.',
  tr: 'Mobil ataletsel algılamadan göreli hareket ve rota bilgisi.',
},

yayaProblemKicker: {
  en: 'THE PROBLEM',
  tr: 'PROBLEM',
},

yayaProblemTitle: {
  en: 'Indoor Movement Is Difficult to Observe Where GNSS Is Unavailable.',
  tr: 'GNSS Kullanılamadığında Kapalı Alan Hareketini Gözlemlemek Zordur.',
},

yayaProblemText: {
  en: 'Large hospitals, warehouses and complex facilities are difficult environments for satellite-based positioning. This makes it harder to understand routes, coverage and how people move through physical spaces.',
  tr: 'Büyük hastaneler, depolar ve karmaşık tesisler uydu tabanlı konumlama için zorlu ortamlardır. Bu durum rotaların, kapsamanın ve insanların fiziksel alan içinde nasıl hareket ettiğinin anlaşılmasını zorlaştırır.',
},

yayaProblemNote: {
  en: 'Yaya focuses on relative movement and operational movement intelligence rather than absolute indoor positioning.',
  tr: 'Yaya, mutlak bina içi konumlama yerine göreli hareket ve operasyonel hareket bilgisine odaklanır.',
},

yayaScopeTitle: {
  en: 'CAPABILITY',
  tr: 'KABİLİYET',
},

yayaScopeMainTitle: {
  en: 'Movement Intelligence From Mobile Inertial Sensors',
  tr: 'Mobil Ataletsel Sensörlerden Hareket Bilgisi',
},

yayaScopeText: {
  en: 'Yaya turns mobile IMU measurements into relative pedestrian movement and combines this information with spatial and operational context.',
  tr: 'Yaya, mobil IMU ölçümlerini göreli yaya hareketine dönüştürür ve bu bilgiyi mekânsal ve operasyonel bağlamla birleştirir.',
},

yayaCapabilityMovementTitle: {
  en: 'Movement Estimation',
  tr: 'Hareket Kestirimi',
},

yayaCapabilityMovementText: {
  en: 'Pedestrian dead reckoning and inertial processing estimate relative displacement and movement from mobile sensors.',
  tr: 'PDR ve ataletsel veri işleme yöntemleriyle mobil sensörlerden göreli yer değiştirme ve yaya hareketi kestirilir.',
},

yayaCapabilityRouteTitle: {
  en: 'Route & Coverage',
  tr: 'Rota ve Kapsama',
},

yayaCapabilityRouteText: {
  en: 'Movement estimates can be used to analyse routes, coverage and repeated movement patterns inside facilities.',
  tr: 'Hareket kestirimleri tesis içindeki rotaların, kapsamanın ve tekrar eden hareket örüntülerinin analizinde kullanılabilir.',
},

yayaCapabilityContextTitle: {
  en: 'Spatial Context',
  tr: 'Mekânsal Bağlam',
},

yayaCapabilityContextText: {
  en: 'Movement data can be associated with facility layouts and spatial context to produce operational movement insights.',
  tr: 'Hareket verisi tesis yerleşimi ve mekânsal bağlamla ilişkilendirilerek operasyonel hareket bilgisine dönüştürülebilir.',
},

yayaApplicationsKicker: {
  en: 'APPLICATION SURFACES',
  tr: 'KULLANIM ALANLARI',
},

yayaApplicationsTitle: {
  en: 'One Movement Core. Different Indoor Operations.',
  tr: 'Tek Hareket Çekirdeği. Farklı Kapalı Alan Operasyonları.',
},

yayaApplicationsLead: {
  en: 'The same mobile inertial capability is adapted according to the facility, users and operational objective.',
  tr: 'Aynı mobil ataletsel kabiliyet; tesise, kullanıcıya ve operasyonel amaca göre uyarlanır.',
},

yayaHospitalTitle: {
  en: 'Hospital Movement',
  tr: 'Hastane İçi Hareket',
},

yayaHospitalText: {
  en: 'Supports route and movement analysis across complex hospital environments using mobile inertial sensing.',
  tr: 'Mobil ataletsel algılama ile karmaşık hastane ortamlarında rota ve hareket analizini destekler.',
},

yayaDepotTitle: {
  en: 'Depot & Facility Analytics',
  tr: 'Depo & Tesis Analitiği',
},

yayaDepotText: {
  en: 'Transforms personnel movement into route, flow and coverage analytics across warehouse and facility operations.',
  tr: 'Personel hareketini depo ve tesis operasyonlarında rota, akış ve kapsama analizine dönüştürür.',
},

yayaPilotKicker: {
  en: 'CONTROLLED PILOT',
  tr: 'KONTROLLÜ PİLOT',
},

yayaPilotTitle: {
  en: 'Validate the Use Case in the Real Environment.',
  tr: 'Kullanım Senaryosunu Gerçek Ortamda Doğrulayın.',
},

yayaPilotScopeTitle: {
  en: 'Define Scope',
  tr: 'Kapsamı Tanımla',
},

yayaPilotScopeText: {
  en: 'The facility, target users, route or movement objective and pilot criteria are defined.',
  tr: 'Tesis, hedef kullanıcılar, rota veya hareket amacı ve pilot ölçütleri belirlenir.',
},

yayaPilotCollectTitle: {
  en: 'Collect Movement',
  tr: 'Hareketi Topla',
},

yayaPilotCollectText: {
  en: 'Movement data is collected from mobile devices during representative use inside the facility.',
  tr: 'Tesis içindeki temsili kullanım sırasında mobil cihazlardan hareket verisi toplanır.',
},

yayaPilotReportTitle: {
  en: 'Evaluate Results',
  tr: 'Sonuçları Değerlendir',
},

yayaPilotReportText: {
  en: 'Route, coverage and movement findings are evaluated and documented according to the pilot objective.',
  tr: 'Rota, kapsama ve hareket bulguları pilotun amacına göre değerlendirilir ve raporlanır.',
},

yayaFinalCtaTitle: {
  en: 'Evaluate Yaya for Your Indoor Operations.',
  tr: 'Yaya’yı Kapalı Alan Operasyonlarınız İçin Değerlendirelim.',
},

yayaFinalCtaText: {
  en: 'Tell us about your facility, users and the movement or route problem you want to evaluate. We can define an appropriate pilot scope together.',
  tr: 'Tesisinizi, kullanıcılarınızı ve değerlendirmek istediğiniz hareket veya rota problemini paylaşın. Uygun pilot kapsamını birlikte belirleyelim.',
},

yayaCtaButton: {
  en: 'DISCUSS YAYA',
  tr: 'YAYA’YI GÖRÜŞELİM',
},
  
  /* --------------------
     PRIVACY / DATA NOTICE
  -------------------- */

  privacyTitle: {
    en: 'Privacy & Data Notice',
    tr: 'Gizlilik ve Veri Bildirimi',
  },

  privacyIntro: {
    en: 'Ayrota collects limited personal data solely for evaluation purposes.',
    tr: 'Ayrota, yalnızca iletişim ve değerlendirme süreçleri için gerekli olan sınırlı kişisel verileri toplar.',
  },

  privacyParagraph1: {
    en: 'Data collected via the contact form may include organization or project name, contact email address, and high-level inquiry context.',
    tr: 'İletişim formu üzerinden ad veya kurum bilgisi, e-posta adresi ve ilettiğiniz talebe ilişkin genel bilgiler toplanabilir.',
  },

  privacyParagraph2: {
    en: 'This website does not perform tracking, profiling, or marketing analytics.',
    tr: 'Bu web sitesinde kullanıcı takibi, profilleme veya pazarlama amaçlı analiz yapılmaz.',
  },

  privacyParagraph3: {
    en: 'Form submissions are processed through a third-party form handling service (Basin) and stored securely for review.',
    tr: 'İletişim formu üzerinden gönderilen bilgiler, üçüncü taraf form hizmeti Basin aracılığıyla işlenir ve talebinizin değerlendirilmesi amacıyla güvenli şekilde saklanır.',
  },

  privacyParagraph4: {
    en: 'Personal data is retained only as long as necessary to assess potential engagement and is deleted periodically.',
    tr: 'Kişisel veriler yalnızca talebin veya olası iş birliğinin değerlendirilmesi için gereken süre boyunca saklanır ve belirli aralıklarla silinir.',
  },

  privacyParagraph5: {
    en: 'Where applicable, data handling complies with KVKK and GDPR.',
    tr: 'Kişisel verilerin işlenmesinde, uygulanabildiği ölçüde KVKK ve GDPR hükümleri gözetilir.',
  },

  privacyConsent: {
    en: 'By submitting the contact form, you acknowledge and consent to this data processing for evaluation purposes.',
    tr: 'İletişim formunu göndererek paylaştığınız bilgilerin, talebinizin değerlendirilmesi amacıyla bu kapsamda işlenmesini kabul etmiş olursunuz.',
  },

  privacyCookieTitle: {
    en: 'Cookie Notice',
    tr: 'Çerez Bildirimi',
  },

  privacyCookieText: {
    en: 'This website does not use tracking or marketing cookies. Any technical cookies used are strictly required for basic functionality and security.',
    tr: 'Bu web sitesinde takip veya pazarlama amacıyla çerez kullanılmaz. Kullanılması halinde teknik çerezler yalnızca sitenin temel işlevleri ve güvenliği için kullanılır.',
  },

} as const;

export type TranslationKey = keyof typeof translations;
